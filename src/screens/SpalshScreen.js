    ...
    <View style={styles.container}>
      ...
      >
        {onBoardingData.map((item, index) => {
          ...
          <View key={index} style={[styles.splashView, {backgroundColor: item.backgroundColor}]}>
              {item.showButton && (
              <RNBounceable onPress={onPressNext} style={{zIndex: 999, position: 'absolute', right: 15}}>
                <Button iconPostionRight={true} useIcon={true} title={item.buttonText} icon="long-arrow-right" onPress={() => navigation.navigate('Log')}/>
              </RNBounceable>
              )}
            <Animatable.View animation={'zoomIn'} style={[styles.imageContainer]}>
              <Animated.Image style={{flex: 1, aspectRatio: 1, transform: [{ scale }]}} source={{uri: item.image}}/>
            </Animatable.View>
            <Animatable.View animation={'slideInUp'} View style={styles.quoteContainer}>
              <Animated.Text  style={[styles.quoteText, {transform: [{ scale }]}]}>{item.quote}</Animated.Text>
              <View style={styles.quoteByContainer}>
                <Animated.Text style={[styles.quoteBy, {transform: [{ scale }]}]}>- {item.quoteBy}</Animated.Text>
              </View>
            </Animatable.View>
          </View>
        )})}
